...
var isDelete = function(){	
	...
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/board/animal/dog/movie/commentList.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	$(document).on('click','.commentDelete',function(){
		...
		$.ajax({
			url:"<c:url value='/board/animal/dog/movie/commentDelete.awa'/>",
	...
	$(function(){	//페이지 로드시 코멘트 목록 뿌려주기
		...
		$('#submit').click(function(){
			check();
			if($(this).html()=='댓글')
				var action="<c:url value='/security/animal/dog/movie/commentWrite.awa'/>";
			else
				var action="<c:url value='/security/animal/dog/movie/commentEdit.awa'/>";
...
<div class="horizontal" style="margin-top: 20px; text-align: right;">
	...
	<a
		href="<c:url value='/dog/movie/List.aw?no=${dto.no}&nowPage=${param.nowPage}'/>"
