...
<c:choose>
	...
	<c:when test="${WHERE eq 'EDT' }">
		...
		<c:set var="successMoveUrl" value="/security/animal/dog/quest/quest_view.aw?no=${param.no}"/>
	</c:when>
	<c:otherwise>
		<c:set var="successMessage" value="삭제 성공했어요"/>
		<c:set var="failMessage" value="삭제 실패했어요"/>
		<c:set var="successMoveUrl" value="/security/animal/dog/quest/quest_list.aw"/>	
	</c:otherwise>
</c:choose>
