...
<script>
	var isDelete = function(){
		if(confirm("글을 삭제 하시겠습니까?"))
			location.replace("<c:url value='/board/animal/dog/tip/delete.aw?no=${record.no}'/>");
	};
	...
	$(function(){
	    ...
	    $(document).one('click', '.like-review', function(e) {
	    	$.ajax({
	    		data : {no : data},
	    		url : "<c:url value='/animal/dog/tip/tip_hit.aw'/>",
	    		type :"POST",
	    		success : function(){
	    			if(text == "♡"){
						hit = hit+1;
					...
					<div class="article__title">
						...
						<div id="btn_e_d_list">
							...
							<c:if test="${sessionScope.mem_no==record.mem_no }">
								<a href="<c:url value='/security/board/animal/dog/tip/edit.aw?no=${record.no}'/>" class="article-action__button button">수정</a>
