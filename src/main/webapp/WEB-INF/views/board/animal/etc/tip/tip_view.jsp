...
<script>
	...
	$(function(){
	    ...
	    $(document).one('click', '.like-review', function(e) {
	    	$.ajax({
	    		data : {no : data},
	    		url : "<c:url value='/animal/etc/tip/tip_hit.aw'/>",
	    		type :"POST",
	    		success : function(){
	    			if(text == "♡"){
						hit = hit+1;
...
//해당 글번호에 대한 코멘트 목록을 가져오는 함수 
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/animal/etcTip/cmt_list.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	var displayComments	 = function(data){
		...
		$('.commentDelete').click(function(){			
			...
			$.ajax({
				url:"<c:url value='/animal/etcTip/cmt_delete.awa'/>",
	...
	$(function(){
		...
		$('#submit').click(function(){	
			if($(this).val()=='등록')
				var action="<c:url value='/animal/etcTip/cmt_write.awa'/>";
			else
			    var action = "<c:url value='/animal/etcTip/cmt_edit.awa'/>";
					...
					<div class="article__title">
						...
						<div id="btn_e_d_list">
							...
							<c:if test="${sessionScope.mem_no==record.mem_no }">
								<a href="<c:url value='/security/board/animal/etc/tip/edit.aw?no=${record.no}'/>" class="article-action__button button">수정</a>
