	...
	<div>
		<form action='<c:url value="/security/animal/etc/quest/quest_write.aw"/>'>
			<div class="offset-sm-9 col-sm-3" style="padding: 5px;padding-right: 0px">	
				<div align="right">
					<a href="<c:url value='/security/animal/etc/quest/quest_write.aw'/>"class="btn btn-success">글쓰기</a>
				</div>
			</div>
		</form>
	</div>
				...
				<c:if test="${not isEmpty }">
					<c:forEach var="record" items="${list}" varStatus="loop">
						<tr>
							...
							<td>
								<a style="color:black;" href="<c:url value='/animal/etc/quest/quest_view.aw?no=${record.dto.no}'/>">${record.dto.quest_title}
