...
<script>
	...
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/bird/quest/cmtList.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	var displayComments	 = function(data){
		...
		$('.commentDelete').click(function(){			
			...
			$.ajax({
				url:"<c:url value='/bird/quest/cmtDelete.awa'/>",
	...
	$(function(){
		...
		$('#submit').click(function(){	
			if($(this).val()=='등록')
				var action="<c:url value='/security/bird/quest/cmtWrite.awa'/>";
			$.ajax({
				url:action,
				data:$('#frm').serialize(),
				dataType:'text',
...
<script>
	$(function(){
		$(function(){
			...
			$('#delete').on('click',function(){
				if(confirm('삭제 하시겠습니까')){
					location.replace("<c:url value='/animal/bird/quest/quest_delete.aw?checking=${record.checking}&no=${record.no}'/>");
				}
			});
		});
		...
		$('.img1').on('click',function(){
			$.ajax({
				data : {no:data},
				url : "<c:url value='/animal/bird/quest/quest_hit.aw'/>",
				type : 'post',
				success: function(){
					hit = hit+1;
	...
	<div class="row" style="padding: 10px;padding-bottom: 0px;text-align:right;padding-right:0px" >
		...
		<div class="offset-sm-3 col-sm-3" style="text-align:right">
			<c:if test="${sessionScope.mem_no == record.mem_no}">
				<a class="text-right" href="<c:url value='/security/animal/bird/quest/quest_edit.aw?no=${record.no}&checking=${record.checking}'/>">수정 &nbsp;</a>
			</c:if>
