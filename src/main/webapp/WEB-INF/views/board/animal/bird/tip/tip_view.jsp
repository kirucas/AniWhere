...
<script>
	var isDelete = function(){
		if(confirm("글을 삭제 하시겠습니까?"))
			location.replace("<c:url value='/board/animal/bird/tip/delete.aw?no=${record.no}'/>");
	};
	...
	$(function(){
	    ...
	    $(document).one('click', '.like-review', function(e) {
	    	$.ajax({
	    		data : {no : data},
	    		url : "<c:url value='/animal/bird/tip/tip_hit.aw'/>",
	    		type :"POST",
	    		success : function(){
	    			if(text == "♡"){
						hit = hit+1;
...
//해당 글번호에 대한 코멘트 목록을 가져오는 함수 
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/animal/birdTip/cmt_list.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	var displayComments	 = function(data){
		...
		$('.commentDelete').click(function(){			
			...
			$.ajax({
				url:"<c:url value='/animal/birdTip/cmt_delete.awa'/>",
	...
	$(function(){
		...
		$('#submit').click(function(){	
			if($(this).val()=='등록')
				var action="<c:url value='/animal/birdTip/cmt_write.awa'/>";
			else
			    var action = "<c:url value='/animal/birdTip/cmt_edit.awa'/>";
