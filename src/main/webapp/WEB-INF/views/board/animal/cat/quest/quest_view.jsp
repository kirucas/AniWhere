...
<script>
	...
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/cat/quest/cmtList.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	$(function(){
		...
		$('#submit').click(function(){	
			if($(this).val()=='등록')
				var action="<c:url value='/security/cat/quest/cmtWrite.awa'/>";
			$.ajax({
				url:action,
				data:$('#frm').serialize(),
				dataType:'text',
	...
	$(function(){
		...
		$('#delete').on('click',function(){
			if(confirm('삭제 하시겠습니까')){
				location.replace("<c:url value='/animal/cat/quest/quest_delete.aw?checking=${record.checking}&no=${record.no}'/>");
			}
		});
		//추천 수 ajax
		...
		$('#fa1').on('click',function(){
			$.ajax({
				data : {no:data},
				url : "<c:url value='/animal/cat/quest/quest_hit.aw'/>",
				type : 'post',
				success: function(){
					hit = hit+1;
...
<div class="container border">
	...
	<div class="row" style="padding: 10px;padding-bottom: 0px;text-align:left;padding-right:0px" >
		...
		<div class="offset-md-3 col-md-3" style="text-align:right">
			...
			<a href="<c:url value='/animal/cat/quest/quest_list.aw'/>"> &nbsp;&nbsp;목록</a>
			<a href="<c:url value='/security/animal/cat/quest/quest_reply.aw?no=${record.no }'/>">|&nbsp;&nbsp;답변</a>
		</div>
	</div>
