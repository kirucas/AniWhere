...
@Controller
public class RnAMovieController {

	private static final int RNA = 3;
	...
	public String list(@RequestParam Map map, // 검색용 파라미터 받기
		...
		String pagingString = PagingUtil.pagingBootStrapStyle(totalRecordCount, PAGESIZE, blockPage, nowPage,
				req.getContextPath() + "/rNa/movie/List.aw?");
		// 데이타 저장]
		...
		// 뷰정보반환]
		return "forward:/animal/rNa/movie.aw";
	}//////////////// list()

	// 등록 폼으로 이동]
	@RequestMapping(value = "/security/animal/rNa/movie/Write.aw", method = RequestMethod.GET)
	public String movie_write() throws Exception {
		return "board/animal/rNa/movie/movieWrite_form.tiles";
	}///////////////////////////

	// 입력처리용]
	@RequestMapping(value = "/security/animal/rNa/movie/Write.aw", method = RequestMethod.POST)
	public String write(@RequestParam Map map) throws Exception {
		
		// 서비스 호출
		service.insert(map);
		// 뷰정보 반환
		return "forward:/rNa/movie/List.aw";
	}

	// 상세보기]
	@RequestMapping("/rNa/movie/view.aw")
	public String movie_view(@RequestParam Map map, Model model) throws Exception {
		...
		// 뷰정보 반환]
		return "board/animal/rNa/movie/movie_view.tiles";
	}/////////////////////

	//수정폼으로 이동 및 수정 처리]
		@RequestMapping("/security/animal/rNa/movie/edit.aw")
		public String movie_edit(Model model, @RequestParam Map map, HttpServletRequest req) throws Exception{
			if(!req.getMethod().equals("POST")) {
				...
				model.addAttribute("dto", dto);
				//수정 폼으로 이동]
				return "board/animal/rNa/movie/movieEdit_form.tiles";
			}
			...
			return "/board/animal/rNa/movie/movieMessage";
		}//////////////////
	
	//삭제 처리]
		@RequestMapping("/rNa/movie/delete.aw")
	...
	//코멘트 입력처리]
			@ResponseBody
			@RequestMapping(value="/security/animal/rNa/movie/commentWrite.awa",produces="text/plain; charset=UTF-8")
			...
			@ResponseBody
			@RequestMapping(value="/board/animal/rNa/movie/commentList.awa",produces="text/html; charset=UTF-8")
			...
			//코멘트 수정 처리
			@ResponseBody
			@RequestMapping(value="/security/animal/rNa/movie/commentEdit.awa",produces="text/html; charset=UTF-8")
			...
			//코멘트 삭제처리]
			@ResponseBody
			@RequestMapping(value="/board/animal/rNa/movie/commentDelete.awa",produces="text/html; charset=UTF-8")
