...
public class EtcTipController {
	...
	//댓글 목록
	@RequestMapping("/board/animal/etc/tip/list.aw")
	public String list(Model model,
			HttpServletRequest req,//페이징용 메소드에 전달
			@RequestParam Map map,//검색용 파라미터 받기
			@RequestParam(required=false,defaultValue="1") int nowPage//페이징용 nowPage파라미터 받기용
			)throws Exception {
		map.put("ani_category",5);
		//서비스 호출]
		//페이징을 위한 로직 시작]
		//전체 레코드 수
		...
		//뷰정보 반환]
		return "board/animal/etc/tip/tip_list.tiles";
		
	}////////// tip_main
	
	//상세보기
	@RequestMapping("/animal/etc/tip/tip_view.aw")
	public String view(@RequestParam Map map,Model model) throws Exception {
		...
		//뷰정보 반환]
		return "board/animal/etc/tip/tip_view.tiles";
	}////////// tip_main
	//등록 폼으로 이동 및 입력처리]
	@RequestMapping(value="/security/animal/etc/tip/write.aw",method=RequestMethod.GET)
	public String write() throws Exception{
		return "board/animal/etc/tip/tip_write.tiles";
	}////////////////
	//입력처리]
	@RequestMapping(value="/security/animal/etc/tip/write.aw",method=RequestMethod.POST)
	...
	//수정폼으로 이동 및 수정 처리]
	@RequestMapping("/security/board/animal/etc/tip/edit.aw")
	...
	//Summernote 업로드 기능
	@ResponseBody
    @RequestMapping(value="/animal/etc/tip/Upload.aw")
