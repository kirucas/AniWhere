...
public class BirdPhotoController {
	...
	//등록 폼으로 이동]
	@RequestMapping(value="/security/{category}/photo/write.aw",method=RequestMethod.GET)
	public String write(Model model,@PathVariable String category) throws Exception{
		// 게시판 분별용
	...
	// 등록 완료 후 리스트로 이동
	@ResponseBody
	@RequestMapping(value="/security/{category}/photo/write.awa",method=RequestMethod.POST,produces="text/html;charset=UTF-8")
	public void writeComplete(@RequestParam Map map,MultipartHttpServletRequest mhsr,HttpSession session,@PathVariable String category) throws Exception{
		int category_no=0;
		switch (category) {
		...
		map.put("ani_category", category_no);
		...
		service.insert(map);
		// 이 메소드만 부르면 될것	
		List<String> upFiles=AwsS3Utils.uploadFileToS3(mhsr,category);
	...
	@RequestMapping("/animal/{category}/photo.aw")
	public String list(@RequestParam Map map,@RequestParam(required=false,defaultValue="1") int nowPage,
			Model model,HttpServletRequest req,@PathVariable String category) throws Exception {
		int category_no=0;
		switch (category) {
			case "dog":category_no=1;break;
			case "cat":category_no=2;break;
...
*/			}/// for
		...
		String pagingString = PagingUtil.pagingBootStrapStyle(totalRecordCount, pageSize, blockPage, nowPage,
				req.getContextPath() + "/animal/"+category+"/photo.aw?");
		// 데이타 저장]
...
*/	
	@RequestMapping("/{category}/photo/delete.aw")
	public String delete(@RequestParam Map map,@PathVariable String category) throws Exception {
		//System.out.println(map.get("no"));
		...
		AwsS3Utils.s3ReadObjects(category);
		return "forward:/animal/"+category+"/photo.aw";
	}/// delete
	
	@ResponseBody
	@RequestMapping(value="/{category}/photo/modalView.awa",method=RequestMethod.POST,produces="text/plain;charset=UTF-8")
	...
	@ResponseBody
	@RequestMapping(value="/{category}/photo/modalContent.awa",method=RequestMethod.POST,produces="text/plain;charset=UTF-8")
	public String modalContent(@RequestParam Map map,HttpSession session,@PathVariable String category) throws Exception {
		//System.out.println("글번호 " +map.get("no"));
