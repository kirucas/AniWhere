...
var isDelete = function(){	
		if(confirm("정말로 삭제 하시겠습니까?"))
			location.replace("<c:url value='/rNa/movie/delete.aw?no=${dto.no}'/>");
	}; 
	...
	var showComments = function(key){		
		$.ajax({
			url:"<c:url value='/board/animal/rNa/movie/commentList.awa'/>",
			data:{no:key},
			dataType:'json',
			type:'post',
	...
	$(document).on('click','.commentDelete',function(){
		...
		$.ajax({
			url:"<c:url value='/board/animal/rNa/movie/commentDelete.awa'/>",
	...
	$(function(){	//페이지 로드시 코멘트 목록 뿌려주기
		...
		$('#submit').click(function(){
			...
			if($(this).html()=='댓글')
				var action="<c:url value='/security/animal/rNa/movie/commentWrite.awa'/>";
			else
				var action="<c:url value='/security/animal/rNa/movie/commentEdit.awa'/>";
...
<div class="horizontal" style="margin-top: 20px; text-align: right;">
	<c:if test="${sessionScope.mem_no == dto.mem_no}">
		<a
			href="<c:url value='/security/animal/rNa/movie/edit.aw?no=${dto.no}'/>"
			class="btn btn-success">수정</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="javascript:isDelete()" class="btn btn-success">삭제</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		</c:if>
	<a
		href="<c:url value='/rNa/movie/List.aw?no=${dto.no}&nowPage=${param.nowPage}'/>"
